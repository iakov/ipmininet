...
from ipaddress import IPv4Address, IPv4Interface, IPv6Address, IPv6Interface, ip_address, ip_network
...
def check_as_path(as_path_rib: str, as_path_us: ExaList):
    ...
    for idx, asn_received, asn_expected in zip(range(len(as_rib)), as_rib, as_rib_us, strict=False):
        assert asn_received == asn_expected, f"Bad ASN at index {idx}. Expected AS{asn_expected}. Received AS{asn_received}"
